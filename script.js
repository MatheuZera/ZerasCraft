...
    // =====================================
    ...
    const textToType = "Construindo uma comunidade épica.";
    let charIndex = 0;
    ...
    const maxTilt = 20; // Grau máximo de inclinação
    ...
    const closeButtons = modal ? modal.querySelectorAll('.modal-close-btn') : [];
    ...
    if (shareButton && socialMenu) {
        shareButton.addEventListener('click', () => {
            // Toggle the visibility by toggling a CSS class
            socialMenu.classList.toggle('hidden');
            socialMenu.style.opacity = socialMenu.classList.contains('hidden') ? 0 : 1;
            
            // Example to hide on click outside
            document.addEventListener('click', (e) => {
                if (!shareButton.contains(e.target) && !socialMenu.contains(e.target)) {
